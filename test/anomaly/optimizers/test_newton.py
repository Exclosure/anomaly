...
def test_linear_solve():
  A, b = jnp.array(1.), jnp.array(1.)
  np.testing.assert_almost_equal(linear_solve(A, b), 1.0)

  A2, b2 = jnp.array([[2., 0.], [0., 1.]]), jnp.array([2., 1.])
  np.testing.assert_array_almost_equal(linear_solve(A2, b2), jnp.array([1., 1.]))
  np.testing.assert_array_almost_equal(linear_solve(A2, b2.reshape((-1,1))), jnp.array([[1., 1.]]).T)

  jit_solve = jax.jit(linear_solve)
  np.testing.assert_array_almost_equal(jit_solve(A2, b2), jnp.array([1., 1.]))
  np.testing.assert_array_almost_equal(jit_solve(A2, b2.reshape((-1,1))), jnp.array([[1., 1.]]).T)
