...
def test_round_trips_jacrev(func_pair, anomaly_and_eccentricity):
  ...
  forward, backward = func_pair
  intermediate = jax.vmap(forward, in_axes=(0, 0))(anomaly, eccentricity)
  g_fwd = jax.vmap(jax.jacrev(forward, argnums=(0,)), in_axes=(0, 0))(anomaly, eccentricity)
  g_bwd = jax.vmap(jax.jacrev(backward, argnums=(0,)), in_axes=(0, 0))(intermediate, eccentricity)
  np.testing.assert_array_almost_equal(g_fwd[0], 1./g_bwd[0], decimal=REQUIRED_DECIMALS)
